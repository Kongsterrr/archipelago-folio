...
export class Game {
 constructor(canvas,inputs,settings,discovery=null){
  ...
  this.time=0;this.accumulator=0;this.wakes=[];this.snapshot=null;this.pendingResume=null;this.focus=null;this.interactables=[];this.actionMarkers=[];this.loadedCount=0;this.quadBike=null;this.quadRideMarker=null;
 ...
 updateNearby(dt,now,frozen){
  ...
  if(this.quadBikeSign&&this.quadBike&&!this.quadBikeSign.userData.anchor){this.quadBikeSign.userData.anchor=this.quadBikeSpawn;}
 ...
 updateCamera(dt){const actor=this.activeActor,walking=this.player.onLand,position=this.player.ridingQuad?this.quadBike.mountPoint.getWorldPosition(vec()):walking?this.jack.root.position:this.visualPosition;
  this.cameraRig.update(dt,{position:this.focus?.boatStudio?this.visualPosition:position,velocity:actor.velocity,yaw:actor.yaw,speed:this.frozen?0:actor.speed,input:walking&&!this.player.ridingQuad?{}:this.inputs.read(actor.yaw,this.cameraRig.yaw),focus:this.focus,locomotion:walking?'walking':'sailing'});
 }
 resetCamera(){const p=this.boat.position;this.visualPosition.set(p.x,p.y,p.z);this.prev.copy(this.visualPosition);this.prevYaw=this.boat.yaw;const a=this.activeActor,position=this.player?.ridingQuad?this.quadBike.mountPoint.getWorldPosition(vec()):a.position;this.cameraRig.lastPoint=null;this.cameraRig.update(0,{position,yaw:a.yaw,locomotion:this.player?.onLand?'walking':'sailing'},true);}
 updateOcclusion(now){
 ...
 teleport(point){
  const wasHidden=this.player?.pauseReasons.has('hidden');this.boarding?.cancel();if(this.quadBike)this.quadBike.park(true);this.player?.toSailing();this.player?.pauseReasons.clear();this.boat.park?.(false);this.cancelChallenge();this.fleet?.onTravel();this.marine?.onTravel();this.docks?.reset();this.focus=null;this.mode='exploring';this.props.resetNear(point);point=this.safePoint(point);this.boat.teleport(point);this.accumulator=0;this.clearWake();this.feedback.clear();this.resetCamera();this.updateNearby(0,performance.now(),false);this.inputs.setEnabled(true);if(wasHidden)this.pause('hidden');this.last=performance.now();
 }
 safePoint(point){
 ...
 reset(){this.boarding?.cancel();if(this.player?.ridingQuad&&this.quadBike){this.quadBike.park(false);this.quadBike.teleport(this.quadBikeSpawn);this.player.pauseReasons.clear();this.mode='exploring';this.focus=null;this.inputs.setEnabled(true);this.resetCamera();return this.player.island;}if(this.player?.walking){this.character.teleport(this.player.berth.landing);this.jack?.resetPose('idle');this.player.pauseReasons.clear();this.mode='exploring';this.focus=null;this.inputs.setEnabled(true);this.resetCamera();return this.player.island;}const i=nearestIsland(this.boat.position);this.teleport({...i.dock,yaw:i.yaw});return i;}
 ...
 async prepareAshore(island){this.inputs.setEnabled(false);this.events.trigger('message',['Preparing '+island.name+' for a little walk…']);
  ...
  if(!this.walkWorlds.has(island.id)){const walk=new IslandWalkWorld(RAPIER,this.world,island,layout);this.walkWorlds.set(island.id,walk);this.createLandActions(island,layout);this.world.propagateModifiedBodyPositionsToColliders();this.world.updateSceneQueries();}const walk=this.walkWorlds.get(island.id);if(island.id==='projects')await this.installQuadBike(island,walk);return walk;
 }
 async installQuadBike(island,walk){
  ...
  this.quadLoading=(async()=>{
   try{
    const quality=this.settings.quality==='low'?'low':'high',path=quality==='low'?'/models/low/quad-bike.glb?v=12':'/models/quad-bike.glb?v=12',gltf=await this.loader.loadAsync(path);
    ...
    if(!quadFootprintClear(walk,p,p.yaw)){
     const alternatives=[[8.5,-8.75],[8.2,-9.35],[9,-8.95],[8.8,-9.35],[9.2,-8.7]];let spawn=null;
     ...
     if(!spawn)throw new Error('No clear parking pad beyond the Research footbridge.');
    ...
    this.createQuadBikePad(island,p);this.quadRideMarker=label('E · RIDE',{width:220,height:72,worldWidth:2.3,background:'#254f62',color:'#fff5dd',fontSize:31});this.scene.add(this.quadRideMarker);this.quadRideMarker.visible=false;
 ...
 createQuadBikePad(island,spawn){
  const group=new THREE.Group();group.name='Projects · trail bike turnout';group.position.set(spawn.x,spawn.y-.02,spawn.z);group.rotation.y=island.rotation;this.scene.add(group);
  ...
  const marker=label('TRAIL BIKE',{width:240,height:68,worldWidth:2.7,background:'#254f62',color:'#fff5dd',fontSize:28});marker.position.set(spawn.x,spawn.y+2.9,spawn.z);this.scene.add(marker);this.quadBikeSign=marker;
 }
 ...
 rideQuad(){if(this.frozen||!this.player.walking||!this.canRideQuad||!this.quadBike)return false;this.cancelChallenge();this.inputs.setEnabled(false);this.character.hold();this.quadBike.park(false);this.quadBike.hold();if(!this.player.rideQuad(this.quadBike)){this.quadBike.park(true);this.inputs.setEnabled(true);return false;}this.quadBike.yaw=this.character.yaw;this.quadBike.body.setRotation({x:0,y:Math.sin(this.quadBike.yaw/2),z:0,w:Math.cos(this.quadBike.yaw/2)},true);this.quadBike.hold();this.inputs.setEnabled(true);this.resetCamera();this.events.trigger('locomotion',['riding-quad']);this.events.trigger('message',['WASD to ride · Shift to boost · E to get off.']);return true;}
 ...
 async reloadQuadQuality(level){if(!this.quadBike||this.quadReloading)return;this.quadReloading=true;try{const quality=level==='low'?'low':'high',path=quality==='low'?'/models/low/quad-bike.glb?v=12':'/models/quad-bike.glb?v=12',gltf=await this.loader.loadAsync(path);this.quadBike.setModel(gltf.scene,quality);}catch(error){console.warn('Quad bike quality switch failed',error.message);}finally{this.quadReloading=false;}}
