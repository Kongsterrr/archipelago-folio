...
export class WakePool{
 constructor(scene,settings,capacity=180){this.settings=settings;this.capacity=capacity;this.items=[];this.last=new Map();this.dummy=new THREE.Object3D();this.mesh=new THREE.InstancedMesh(new THREE.CircleGeometry(1,10),new THREE.MeshBasicMaterial({color:'#d7f6ee',transparent:true,opacity:.3,depthWrite:false}),capacity);this.mesh.frustumCulled=false;scene.add(this.mesh);this.update(0,0);}
