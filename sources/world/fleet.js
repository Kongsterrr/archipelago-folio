...
export class AmbientFleet{
 ...
 async load(){
  ...
  for(const route of FLEET_ROUTES){const assets=models.get(route.kind);if(!assets)continue;const p=routePoint(route,route.phase),yaw=routeYaw(route,route.phase);
   ...
   const collider=this.world.createCollider(this.R.ColliderDesc.cuboid(route.width*.43,1,route.length*.43).setCollisionGroups(0x00010001).setMass(route.kind==='cargo'?32:route.kind==='yacht'?24:18).setFriction(.04).setRestitution(.04).setActiveEvents(this.R.ActiveEvents.CONTACT_FORCE_EVENTS).setContactForceEventThreshold(1),body);
