...
const loader=new GLTFLoader().setMeshoptDecoder(MeshoptDecoder),gltf=await loader.loadAsync(`/models/${kind}.glb?v=${kind==='jack'?8:7}`),surfaces=new SurfaceLibrary({renderer});
...
renderer.setAnimationLoop(render);document.querySelector('h1').textContent=`V8 · ${kind.toUpperCase()} / ${clay?'CLAY':'ACTUAL MATERIALS'}`;document.querySelector('#state').textContent=`Actual runtime GLB · ${clay?'Neutral clay':surfaces.stats().status+' · shared game surfaces'} · Web${renderer.backend.isWebGPUBackend?'GPU':'GL2'}`;
