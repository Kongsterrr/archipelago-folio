...
- Hold Shift to boost, Space to brake. E or Enter opens a nearby dock.
...
- A duck and a message bottle are hidden in the bay.
...
No backend, accounts, remote database, private Bruno services or network leaderboard are used. Browser storage contains only settings and the versioned local best race time, with an in-memory fallback when storage is blocked.
...
See `static/models/compression.json` for the before/after sizes. The ten GLBs total approximately 0.71 MB, down from 1.77 MB. Collisions are intentionally independent of the visual mesh in `sources/core/collisions.js`. Boats use fixed 60 Hz Rapier simulation, planar constraints, CCD and interpolated render positions.
