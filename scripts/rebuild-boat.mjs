...
authored.traverse(object=>{
 ...
 const geometry=object.geometry.clone();for(const name of Object.keys(geometry.attributes))if(!['position','normal','color'].includes(name))geometry.deleteAttribute(name);
...
const model={id:'boat',name:'boat',file:'boat.glb',revision:'v3-marina-runabout',bytes:(await fs.stat(output)).size,rawBytes:raw.byteLength,sourceMeshes,drawCalls:primitives.length,staticDrawCalls:primitives.length-animationNodes.length,vertices:primitives.reduce((s,p)=>s+p.getAttribute('POSITION').getCount(),0),triangles:primitives.reduce((s,p)=>s+(p.getIndices()?.getCount()||p.getAttribute('POSITION').getCount())/3,0),bounds:{min:bounds.min.toArray(),max:bounds.max.toArray()},dimensions:bounds.getSize(new THREE.Vector3()).toArray(),animationNodes,nodes:animationNodes};
