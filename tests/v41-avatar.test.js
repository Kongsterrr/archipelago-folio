...
test('rebuilt proportions keep the original boat-space wheel grip anchors',async()=>{const f=await fixture();const targets=[new THREE.Vector3(.19303,.1615,-.21631),new THREE.Vector3(.37345,.1615,-.21631)];f.hands().forEach((p,i)=>assert.ok(p.distanceTo(targets[i])<.0001,'wheel contact within 0.1mm compression tolerance'));});
